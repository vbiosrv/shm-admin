...
function PaymentSystems() {
  ...
  const loadPaymentSystems = async () => {
    ...
    try {
      ...
      // Если API вернул массив в data
      if (Array.isArray(response?.data)) {
        systems = response.data;
      } 
      // Если API вернул сразу массив
      else if (Array.isArray(response)) {
        systems = response;
      }

      // Фильтр на валидные объекты и добавляем defaults
      const activeSystems: PaymentSystem[] = systems.map(system => ({
        name: system.name || system.title || 'Без имени',
        title: system.title || system.name || 'Без названия',
        description: system.description || 'Описание отсутствует',
        price: system.price ?? 0,
        paid: true, // помечаем все как доступные
      }));
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 mr-4">
            ...
            <button onClick={() => setSortType(sortType === 'price-asc' ? 'price-desc' : 'price-asc')}
            ...
            <button onClick={() => setSortType(sortType === 'paid-first' ? 'unpaid-first' : 'paid-first')}
